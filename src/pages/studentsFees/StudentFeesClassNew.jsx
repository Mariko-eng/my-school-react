...
import { Form, Select, Button, PageHeader, Modal, Col, Row } from "antd";
import axiosInstance from "../../utils/axios";
import { useNavigate } from "react-router-dom";

const StudentFeesClassNew = () => {
  ...
  const submitFees = () => {
    ...
    axiosInstance
      ...
      .catch((error) => {
        console.log(error);
        return Modal.error({
          title: "Fee Already Exists Or Service Not Currently Available",
        });
      });
  };
  ...
  useEffect(() => {
    axiosInstance
      .get("http://127.0.0.1:8000/finance/fees-list/")
      .then((res) => {
    ...
    <>
              ...
              >
                ...
                >
                  {schoolfees.map((item, index) => (
                    <Select.Option key={index} value={item.id}>
                      {item.name} ({item.class_name}) ({item.amount})
                    </Select.Option>
                  ))}
                </Select>
              </Form.Item>
      ...
      {xfees.length > 0 && (
        <div style={{ marginTop: "10px" }}>
          {xfees.map((fee, index) => (
            <Row key={index}>
              <Col span={4}>{fee.type}</Col>
              <Col span={4}>{fee.name}</Col>
...
export default StudentFeesClassNew;
